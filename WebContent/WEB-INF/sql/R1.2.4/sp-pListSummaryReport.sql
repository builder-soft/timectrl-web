...
#	DECLARE vDynamicSQL			VARCHAR(1000);

#	16:15 lunes 25 de mayo, en centro medigo de ernesto albear, hora del dante.
...
#		SET vCurrent = DATE(CONCAT(YEAR(vCurrent), '-', MONTH(vCurrent), '-01'));
...
#		INSERT INTO tTemp(cStart, cEnd, cEmployeeKey, cEmployeeId, cEmployeeName) SELECT vCurrent, LAST_DAY(vCurrent), cKey, cId, cName FROM tEmployee WHERE cBoss=vBoss ORDER BY cName;

		SET @vCurrent = vCurrent;
#		SET @vEmployees = vEmployees;
...
#select @vCurrent, @vCurrent, @vBoss;
#select * from tTemp;
...
#if (NOT vStartDiffI IS NULL) then
#select vStartDiffI, vStartMark, vStartTime;				
#end if;				
