...
BEGIN
	...
	DECLARE vDayDiffI	INTEGER;
	DECLARE vDayDiffM	VARCHAR(9);
...
/**Variables locales */	
	...
	WHILE vCurrent != vEndDate DO
		...
		SET vMachine = (SELECT cMachine 
						FROM tAttendanceLog 
						WHERE cEmployeeKey = vEmployeeKey AND DATE(cDate) = DATE(vCurrent) AND cMarkType=1 LIMIT 1);
		...
		IF(vTurnDayId IS NULL) THEN
			...
			ELSE
				SET vComment = 'Turno diferido';
			END IF;
		END IF;
		
		/**
		 Calculo de hora Extra para el viernes, 8:45 a 15:45 Horario Viernes, si la persona marca 
		 antes de las 15:45 no es atraso y no se descuenta, solo hasta las 14:45, si es menor a 
		 14:45 se marcara descuento por salida anticipada, y el descuento es a partir de las 14:45 
		 no de las 15:45, si marca después de las 15:45 debe marcar extra. 
		 */
		
		SET vDayDiffI = vEndDiffI + vStartDiffI;
		SET vDayDiffM = fExtraTime3(vDayDiffI);
	...
	SELECT  cId, cDate, IFNULL(cStartTime,'') AS cStartTime, IFNULL(cStartMark,'') AS cStartMark, IFNULL(cStartDiffI, '') AS cStartDiffI, 
			IFNULL(cStartDiffM, '') AS cStartDiffM, IFNULL(cLunchMark, '') AS cLunchMark, IFNULL(cReturnMark, '') AS cReturnMark, 
			IFNULL(cEndTime, '') AS cEndTime, IFNULL(cEndMark, '') AS cEndMark, IFNULL(cEndDiffI,'') AS cEndDiffI, IFNULL(cEndDiffM,'') AS cEndDiffM, 
			IFNULL(cDayDiffI,'') AS cDayDiffI, IFNULL(cDayDiffM,'') AS cDayDiffM, 
			cComment, IFNULL(cMachine,'') AS cMachine,
...
CREATE FUNCTION fExtraTimeAsMins3(vMark DATETIME, vLimitTime CHAR(8), vStart BOOLEAN) RETURNS INTEGER
BEGIN
