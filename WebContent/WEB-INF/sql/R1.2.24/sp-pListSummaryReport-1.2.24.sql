...
CREATE PROCEDURE pListSummaryReport(IN vBoss BIGINT(20), IN vEmployees VARCHAR(2000), IN vStartMonth INT, IN vStartYear INT, 
...
BEGIN
	...
	WHILE vCurrent <= vLast DO
		SET @vDynamicSQL = CONCAT('INSERT INTO tTemp(cStart, cEnd, cEmployeeKey, cEmployeeId, cEmployeeRut, cEmployeeName) SELECT ?, LAST_DAY(?), cKey, cId, cRut, cName FROM tEmployee WHERE cId IN (', vEmployees, ') AND tEmployee.cEnabled=TRUE ORDER BY cName;');
...
#select vStartMark, vStartTime, TRUE, vTurnDayId, vStartDiffI;
				...
				IF(vBothMarks) THEN
					...
					END IF;
				ELSE
					IF(NOT vStartMark IS NULL OR NOT vEndMark IS NULL) THEN /* Si Tiene alguna marca (entrada o salida), suma uno */
						SET vMarkCount := vMarkCount + 1;
					END IF;
				END IF;
